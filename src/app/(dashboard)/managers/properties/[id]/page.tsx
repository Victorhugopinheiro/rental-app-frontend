...
import { useGetAuthUserQuery, useGetLeasePaymentsQuery, useGetPropertyLeasesQuery, useGetPropertyQuery } from "@/state/api"
...
import { ArrowDownToLine, ArrowLeft, Check, Download, DownloadCloud } from "lucide-react";
...
export default function ManagerProperties() {
    ...
    const { data: property } = useGetPropertyQuery(propertyId, {
        skip: !propertyId || !authUser?.cognitoInfo.userId,
    })

    const { data: leases } = useGetPropertyLeasesQuery(propertyId, {
        skip: !propertyId || !authUser?.cognitoInfo.userId,
    })



    const { data: payment } = useGetLeasePaymentsQuery(propertyId, {
        skip: !propertyId || !authUser?.cognitoInfo.userId,
    })


    function getCurrentMonthPaymentStatus(leaseId: number) {
        const currentDate = new Date();
        const currentMonth = currentDate.getMonth();
        const currentYear = currentDate.getFullYear();

        const currentMonthPayment = payment?.find(
            (pay) => pay.leaseId === leaseId &&
                new Date(pay.dueDate).getMonth() === currentMonth &&
                new Date(pay.dueDate).getFullYear() === currentYear
        )
